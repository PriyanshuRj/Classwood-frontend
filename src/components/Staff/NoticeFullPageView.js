  ...
  return (
    ...
    <div className='flex p-8 m-8 bg-white rounded-2xl'>
        <div className='flex flex-col items-center justify-center w-full'>
            <p className='mb-4 text-2xl font-semibold underline'>{Notice ? Notice.title : undefined}</p>
           <span className='flex justify-end w-full text-sm'>
               {date}
           </span>
            <p className='mt-2 text-md' >{Notice ? Notice.description : undefined}</p>
        </div>
    </div>
      </Layout>
  )
