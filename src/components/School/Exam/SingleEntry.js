...
import { updatePercentage, updateSubjectMarks, updateTotalMarks, updateMakrsheet } from '../../../store/genralUser';
export default function SingleEntry({student, index}) {
  const dispatch = useDispatch();
    ...
    useEffect(()=>{
      if(student.totalMarks && student.marks){
        ...
        dispatch(updatePercentage({value : ((parseInt(student.marks)/parseInt(student.totalMarks))*100).toFixed(2) , id : index}));
      }
  ...
  return (
    <div  className="grid grid-cols-6 gap-4 mb-2">
        ...
        <input
          type="number"
          onChange={(e)=> dispatch(updateTotalMarks({value:e.target.value,id: index}))}
        ...
        <input
          type="number"
          onChange={(e)=> dispatch(updateSubjectMarks({value:e.target.value,id: index}))}
          value={student.marks}
...
{student.percentage ? 
        <span className=" h-[50px] bg-[#F8FAFC] border-[1px] rounded-lg flex items-center justify-end px-2 text-gray-800">
          {student.percentage} 
        </span>: <span className=" h-[50px] bg-[#F8FAFC] border-[1px] rounded-lg flex items-center justify-end px-2 text-gray-400">
          Percentage
        </span>}
        ...
        <div className="flex items-center justify-center">
          ...
          >
            ...
            <input
              ...
              type="file"
              className="hidden"
              onChange={(e) => dispatch(updateMakrsheet({id:index, value:e.target.files[0]}))}
            />
          </label>
        </div>
      </div>
