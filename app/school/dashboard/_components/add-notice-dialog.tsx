...
export function AddNoticeDialog() {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      const res = await createNotice({ title, description });
      if (!res.ok) {
      ...
      <Dialog open={open} onClose={close} className="relative z-50">
        ...
        <div className="fixed inset-y-0 right-0 flex w-full max-w-md">
          <DialogPanel className="flex h-full w-full flex-col bg-white p-6 shadow-xl">
            <DialogTitle className="text-2xl font-semibold">Add Notice</DialogTitle>

            <label className="mt-6 text-sm font-medium text-gray-700">Title</label>
            <input
              type="text"
              value={title}
